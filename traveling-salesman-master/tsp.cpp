...
TSP::TSP(string in, string out)
{
	...
	// graph is adjacency matrix replresentation
	graph = new int*[n];
	for (int i = 0; i < n; i++) {
		graph[i] = new int[n];
		for (int j = 0; j < n; j++)
...
int TSP::get_distance(struct TSP::City c1, struct TSP::City c2)
{
	...
	return (floor((float) (sqrt(dx + dy)) + 0.5)); // Round off to nearest integer (thus distance is not exact)
};


// Thread Function
void *thread_func(struct thread_data *my_data)
{
	int tid = my_data->tid;
	TSP *_tsp = my_data->tsp;
	int **graph = _tsp->graph;
	int start, end;
	start = _tsp->start_idx[tid];
	end = _tsp->end_idx[tid];

	for (int i = start; i <= end; i++)
	{
		for (int j = i; j < _tsp->n; j++)
		{
			// Build matrix 
			graph[i][j] = graph[j][i] =  _tsp->get_distance(_tsp->cities[i], _tsp->cities[j]);
		}
	}
}
...
void TSP::fillMatrix_threads()
{
	...
	for (long t = 0; t < THREADS; t++) {
		...
		data[t].tid = t;
		data[t].tsp = this;
		rc = pthread_create(&thread[t], &attr, thread_func, (void*)&data[t]);
		if (rc) {
...
void TSP::make_shorter(){
	...
	cout << twoOpt(graph, circuit, pathLength, n) << " --<> \n";
}
...
void TSP::printResult(){
	// cout.open(outFname.c_str(), ios::out);
