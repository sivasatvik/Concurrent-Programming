 ...
 */
node_t* list_search(llist_t* set, my_pair val, node_t** left_node) 
{
...
llist_t* list_new()
{
  ...
  the_list->head = new_node(std::make_pair(std::vector<int>(),INT_MIN), NULL);
  the_list->tail = new_node(std::make_pair(std::vector<int>(),INT_MAX), NULL);
