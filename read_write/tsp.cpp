...
void Genetic::run(int thread_id)
{
	...
	for(int i = 0; i < generations; i++)
	{
		...
		// 		// mtx.lock();
		// 		auto temp = population[0];
		// 		auto temp1 = population[1];
		// 		// mtx.unlock();
		// 		crossOver(temp.first, temp1.first);
		// 	}
		// 	else
		// 	{
				// real_size_population > 2
		if(real_size_population!=1){
				...
				crossOver(temp1.first, temp2.first);
			// }
		...
		else // population contains only 1 parent
		{
			...
			crossOver(temp.first, temp.first);
